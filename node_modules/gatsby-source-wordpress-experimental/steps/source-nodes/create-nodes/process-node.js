...
const fetchNodeHtmlImageMediaItemNodes = async ({
  ...
  if (createdNodeIds.length) {
    const previouslyCreatedNodeIds = (await helpers.cache.get(_constants.CREATED_NODE_IDS)) || [];
    const allCreatedNodeIds = [...createdNodeIds, ...previouslyCreatedNodeIds];

    if (allCreatedNodeIds.length) {
      await helpers.cache.set(_constants.CREATED_NODE_IDS, allCreatedNodeIds);
    }
  }
...
const replaceNodeHtmlImages = async ({
  ...
  const imgTagMatches = (0, _execall.default)(imgTagRegex, nodeString // we don't want to match images inside pre
  .replace(/<pre([\w\W]+?)[\/]?>.*(<\/pre>)/gim, ``) // and code tags, so temporarily remove those tags and everything inside them
  .replace(/<code([\w\W]+?)[\/]?>.*(<\/code>)/gim, ``)).filter(({
    match,
    subMatches
  }) => {
